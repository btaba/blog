...
To help interpret associativity, let's take transforms $T$ and $X$. If $T$ is left-associative, we first rotate $X$ then translate. This implies that left-associative transforms are "space frame transforms", or $T$ gets us into the parent frame of $X$. If $T$ is right-associative, we translate then rotate $X$. This implies that right-associative transforms are "body frame transforms" or $T$ gets us into the child frame of $X$.
